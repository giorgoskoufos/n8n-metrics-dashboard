...
async function loadRoiMetrics() {
    ...
    try {
        const response = await fetchWithAuth('/api/analytics/roi');
        ...
        if (data.topWorkflows && data.topWorkflows.length > 0) {
            tableBody.innerHTML = data.topWorkflows.map(wf => `
                <tr class="hover:bg-n8n-dark/30 transition-colors">
                    ...
                    <td class="py-3 pl-4 text-right text-green-400 font-bold">${formatTimeExtensive(wf.time_saved_seconds)}</td>
                </tr>
            `).join('');
        } else {
            tableBody.innerHTML = `
        ...
        tableBody.innerHTML = `<tr><td colspan="3" class="py-6 text-center text-red-500">Failed to load ROI data. <button onclick="loadRoiMetrics()" class="underline text-red-400 hover:text-white ml-2">Retry</button></td></tr>`;
    }
}
